...
type UserRole {
  ...
  """Role name"""
  role: String!
}`,pn=fI(),Ra=Vs(Ms.DIRECTIVES,[]);var Tf;const ZA=zs((Tf=pn.getQueryType())==null?void 0:Tf.getFields());var Af;const eI=zs((Af=pn.getMutationType())==null?void 0:Af.getFields());var If;const nI=zs((If=pn.getSubscriptionType())==null?void 0:If.getFields()),iu=Ct.keyBy(xp(),i=>i.name.toLocaleLowerCase()),tI=Ct.mapValues(iu,i=>{const n=Ra.find(t=>(t==null?void 0:t.name)===i.name||(t==null?void 0:t.name)==="*");return n?n.args.some(t=>t==="*")?i.args:n.args.map(t=>i.args.find(s=>s.name===t)).filter(t=>!!t):[]}),iI=zs(pn.getTypeMap()),rI=QA(pn);function zs(i){return Ct.mapKeys(i||{},(n,t)=>t.toLocaleLowerCase())}function sI(){return Ct.size(pn.getTypeMap())<=10}function oI(){return[Ia("Queries",pn.getQueryType()),Ia("Mutations",pn.getMutationType()),Ia("Subscriptions",pn.getSubscriptionType()),pI(),lI()].filter(i=>!!i)}function Ia(i,n){return uI(i,aI(n))}function aI(i){return Ct.sortBy((i==null?void 0:i.getFields())||{},n=>n.name)}function uI(i,n){return n.length===0?null:{type:"menu",title:i,children:n.map(t=>({type:"page",title:t.name,section:i,deprecated:!!t.deprecationReason,href:br.joinUrlPaths(Gs(),i.toLocaleLowerCase(),t.name)}))}}function lI(){return sI()?null:{type:"menu",title:"Types",children:Ct.sortBy(Ct.map(pn.getTypeMap()),n=>n.name).filter(n=>!n.name.startsWith("__")).map(n=>({type:"page",title:n.name,section:"Types",href:br.joinUrlPaths(Gs(),"types",n.name)}))}}function wI(){return!!pn.getQueryType()}function FI(i){return ZA[i.toLocaleLowerCase()]}function NI(){return!!pn.getMutationType()}function CI(i){return eI[i.toLocaleLowerCase()]}function OI(){return!!pn.getSubscriptionType()}function kI(i){return nI[i.toLocaleLowerCase()]}function xI(i){return iI[i.toLocaleLowerCase()]}function cI(i){return iu[i.toLocaleLowerCase()]}function RI(i){return cI(i.name)!==void 0}function LI(){return Ct.size(iu)>0}function BI(i){return tI[i.name.toLocaleLowerCase()]||[]}function PI(i){if(i)return rI.getFor(i)}function $I(i){return Ct.flatMap(i.getFields(),n=>({field:n,possibleDescriptions:kp(n,i)}))}function kp(i,n){return i?i.description?[{description:i.description,from:n}]:We(n)?n.getInterfaces().flatMap(t=>kp(t.getFields()[i.name],t)):[]:[]}function xp(){return Ra.some(i=>(i==null?void 0:i.name)==="*")?pn.getDirectives().filter(i=>!["include","skip","deprecated","specifiedBy"].includes(i.name)):Ra.filter(i=>!!(i!=null&&i.name)).map(({name:i})=>i?pn.getDirective(i):void 0).filter(i=>!!i)}function fI(){return yf.trim().length===0?qA(JSON.parse(JSON.stringify({__schema:{types:[]}}))):zA(yf)}function pI(){const i=xp();return i.length===0?null:{type:"menu",title:"Directives",children:i.map(n=>({type:"page",title:n.name,href:br.joinUrlPaths(Gs(),"directives",n.name),section:"Directives"}))}}const UI=Vs(Ms.APP_TITLE,"GraphQL Documentation"),Rp=hI().concat(oI());mI(Rp);const dI=Object.freeze(Rp),MI=gI();function hI(){return Vs(Ms.PAGES,EI()).filter(n=>!!n).map(n=>Bp([],n))}function gI(){const i=vI();if(i)return i.href;throw new Error("No custom pages or query available to use as the root application URL. You need to provide at least one custom page or your schema should contain at least one query/mutation/subscription.")}function mI(i){function n(s,o){for(const a of s){if(a.type==="page"){o(a);continue}a.type==="menu"&&n(a.children,o)}}let t;n(i,s=>{t&&(t.next={title:s.title,section:s.section,href:s.href},s.previous={title:t.title,section:t.section,href:t.href}),t=s})}function vI(){return Lp(()=>!0)}function VI(i){return Lp(n=>n.href.toLocaleLowerCase()===i.toLocaleLowerCase())}function Lp(i){function n(t){for(const s of t){if(s.type==="page"&&i(s))return s;if(s.type==="menu"){const o=n(s.children);if(o)return o}}return null}return n(dI)}function Bp(i,n){if(typeof n.content=="string")return{type:"page",title:n.title,content:n.content,href:br.joinUrlPaths(Gs(),...i,_f(n.title))};const t=i.concat([_f(n.title)]);return{type:"menu",title:n.title,children:n.content.map(s=>({...Bp(t,s),section:n.title}))}}function _f(i){return br.generatePathSegment(i,new Nf)}function EI(){return[{title:"Introduction",content:[{title:"Welcome",content:`
        # Welcome 🎉
        ...
        `.split(`
`).map(i=>i.trim()).join(`
`)}]}]}export{A_ as A,sI as B,xI as C,PI as D,Ht as E,nt as F,Ge as G,Gn as H,We as I,En as J,_ as K,$I as L,hi as M,Bi as N,bI as O,$s as P,wf as Q,gr as R,II as S,Nf as T,De as U,Fn as V,UI as W,Ct as _,NI as a,wI as b,FI as c,OI as d,kI as e,VI as f,CI as g,MI as h,Sf as i,AI as j,LI as k,cI as l,BI as m,Gs as n,RI as o,dI as p,ft as q,np as r,pn as s,Dn as t,br as u,be as v,SI as w,Vs as x,Ms as y,lr as z};
